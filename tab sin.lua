...
tab.newButton("telekinet ", "??", function()
loadstring(game:HttpGet(('https://raw.githubusercontent.com/SAZXHUB/Control-update/main/README.md'),true))()
end)
tab.newButton("bringmobv1", "", function()
local player = game.Players.LocalPlayer

-- Vị trí offset khi bring
local offsets = {
	Vector3.new(-3, 0, 0),
	Vector3.new(3, 0, 0),
	Vector3.new(0, 0, -3),
...
-- Bring trong bán kính 90
local function bringMobs()
	local char = player.Character or player.CharacterAdded:Wait()
	local hrp = char:FindFirstChild("HumanoidRootPart")
	if not hrp then return end

	-- Mở rộng SimulationRadius
	...
	for _, model in ipairs(workspace:GetDescendants()) do
		if model:IsA("Model") and not game.Players:GetPlayerFromCharacter(model) then
			...
			if root and hum and hum.Health > 0 then
				...
				if dist <= 90 then
					local offset = offsets[index] or Vector3.new(math.random(-3, 3), 0, math.random(-3, 3))
...
gui.Name = "BringHumanoidGui"
gui.ResetOnSpawn = false

local bringBtn = Instance.new("TextButton", gui)
bringBtn.Size = UDim2.new(0, 120, 0, 40)
bringBtn.Position = UDim2.new(0, 10, 0, 10)
bringBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
bringBtn.Text = "Bring Once"
...
-- Khi bấm nút thì bring
bringBtn.MouseButton1Click:Connect(function()
	bringMobs()
   end)
end)
tab.newButton("bringmobv2", "all!!!", function()
local player = game.Players.LocalPlayer
local mobControl = {}
...
local function drawLine(part1, part2)
	...
	line.Size = Vector3.new(0.25,0.25,(part1.Position-part2.Position).Magnitude)
...
-- Gắn BodyPosition cho NPC
local function attachToMob(root)
	if mobControl[root] then return end
...
-- Bring NPC trong bán kính 90 với cục đen
local function bringMobs()
	local char = player.Character or player.CharacterAdded:Wait()
	local hrp = char:FindFirstChild("HumanoidRootPart")
	if not hrp then return end
		...
		if model:IsA("Model") and not game.Players:GetPlayerFromCharacter(model) then
			...
			if root and hum and hum.Health > 0 then
				...
				if dist <= 1000 then
					...
					-- Gắn BodyPosition kéo về player + offset
					attachToMob(root)
					mobControl[root].Position = hrp.Position + offset

					-- Vẽ line visual
					local line = drawLine(point, root)
					game:GetService("Debris"):AddItem(line, 0.1) -- tự biến mất nhanh
...
tab.newButton("control mob", "", function()
--[[
    SCRIPT ĐIỀU KHIỂN MOB BẰNG CLICK CHUỘT/CHẠM - BẢN FIX LỖI ĐỨNG HÌNH GIẬT KHỰNG
    - Sửa lỗi giằng co CFrame giữa Player và Mob gây đứng hình khi đi Joystick.
    - Giấu xác chuẩn -10 studs sử dụng ToWorldSpace mượt mà.
    - Cầm tool bay lên, cất tool quái rơi tự do tự nhiên.
]]
...
local Character, MyRoot, MyHumanoid
local targetMob = nil
local targetRoot = nil
local targetHum = nil

-- Cấu hình trạng thái bay
local isFlying = false
local flySpeed = 16 

-- CẤU HÌNH ĐỘ MƯỢT DI CHUYỂN
...
local function UpdateCharacterVars(char)
	...
	MyHumanoid.Died:Connect(function()
		if targetMob then
			if targetRoot and targetRoot:FindFirstChild("MobMoveForce") then
				targetRoot.MobMoveForce:Destroy()
			end
			targetMob = nil
			targetRoot = nil
			targetHum = nil
...
if LocalPlayer.Character then UpdateCharacterVars(LocalPlayer.Character) end
LocalPlayer.CharacterAdded:Connect(UpdateCharacterVars)

local function checkIsMob(target)
	if not target then return nil end
	local model = target:FindFirstAncestorOfClass("Model")
	if model and model ~= Character and not Players:GetPlayerFromCharacter(model) then
...
local function RemoveFlyTool()
	isFlying = false
	local Backpack = LocalPlayer:FindFirstChild("Backpack")
	if Backpack then
		local t = Backpack:FindFirstChild("✈️ Cầm Để Bay Lên")
		if t then t:Destroy() end
	end
	if Character then
		local t = Character:FindFirstChild("✈️ Cầm Để Bay Lên")
		if t then t:Destroy() end
	end
end
...
local function EquipFlyTool()
	...
	if not Backpack:FindFirstChild("✈️ Cầm Để Bay Lên") and not Character:FindFirstChild("✈️ Cầm Để Bay Lên") then
		local flyTool = Instance.new("Tool")
		flyTool.Name = "✈️ Cầm Để Bay Lên"
		...
		flyTool.Equipped:Connect(function()
			isFlying = true
			StarterGui:SetCore("SendNotification", {Title = "Trạng thái", Text = " bay lên..."})
		end)
		
		flyTool.Unequipped:Connect(function()
			isFlying = false
			StarterGui:SetCore("SendNotification", {Title = "Trạng thái", Text = "Đã tắt bay! 👽👽..."})
		end)
	end
end

local function MaintainTool()
	local Backpack = LocalPlayer:WaitForChild("Backpack", 5)
	if not Backpack then return end
	
	if not Backpack:FindFirstChild("🎮 Click Nhập Xác") and not Character:FindFirstChild("🎮 Click Nhập Xác") then
		local controlTool = Instance.new("Tool")
		controlTool.Name = "🎮 Click Nhập Xác"
		controlTool.RequiresHandle = false
		controlTool.CanBeDropped = false
		controlTool.Parent = Backpack
		...
		controlTool.Activated:Connect(function()
			if targetMob then
				...
				StarterGui:SetCore("SendNotification", {Title = "Thoát Xác", Text = "Đã trả tự do cho thân thể!"})
			else
				local clickedMob = checkIsMob(Mouse.Target)
				if clickedMob then
					targetMob = clickedMob
					targetRoot = clickedMob.HumanoidRootPart
					targetHum = clickedMob:FindFirstChildOfClass("Humanoid")
					...
					StarterGui:SetCore("SendNotification", {
						Title = "Nhập Xác", 
						Text = "Đang lái: " .. clickedMob.Name .. "\nCầm thêm Tool Bay để cất cánh!"
					})
				else
					StarterGui:SetCore("SendNotification", {Title = "Thông báo", Text = "Hãy chạm/click thẳng vào để kí sinh!"})
				end
			end
		end)
	end
...
-- Vòng lặp RenderStepped sửa lỗi đứng hình, khựng lực
RunService.RenderStepped:Connect(function()
	if targetMob and targetRoot and MyRoot then
		if sethiddenproperty then
		...
		-- FIX 1: Giấu xác sử dụng ToWorldSpace tính theo góc quay thực và đồng bộ Velocity tránh khựng
		MyRoot.CFrame = targetRoot.CFrame:ToWorldSpace(CFrame.new(0, -7, 0))
		MyRoot.Velocity = targetRoot.Velocity
		
		-- FIX 2: Tắt va chạm triệt để toàn bộ phận cơ thể để không cản trở Mob di chuyển
		...
		local bf = targetRoot:FindFirstChild("MobMoveForce")
		if not bf then
			bf = Instance.new("BodyVelocity")
			bf.Name = "MobMoveForce"
			bf.Parent = targetRoot
		end
		
		if isFlying then
			bf.MaxForce = Vector3.new(999999, 999999, 999999)
			
			if moveDirection.Magnitude > 0 then
				local finalDirection = Vector3.new(moveDirection.X, 0, moveDirection.Z).Unit
				local targetVelocity = Vector3.new(finalDirection.X * mobSpeed, flySpeed, finalDirection.Z * mobSpeed)
				
				bf.Velocity = bf.Velocity:Lerp(targetVelocity, SMOOTHNESS)
				
				local targetCFrame = CFrame.new(targetRoot.Position, targetRoot.Position + finalDirection)
				targetRoot.CFrame = targetRoot.CFrame:Lerp(targetCFrame, SMOOTHNESS)
			else
				bf.Velocity = bf.Velocity:Lerp(Vector3.new(0, flySpeed, 0), SMOOTHNESS)
			end
		else
			bf.MaxForce = Vector3.new(999999, 0, 999999)
			
			if moveDirection.Magnitude > 0 then
				local finalDirection = Vector3.new(moveDirection.X, 0, moveDirection.Z).Unit
				local targetVelocity = Vector3.new(finalDirection.X * mobSpeed, targetRoot.Velocity.Y, finalDirection.Z * mobSpeed)
				...
				bf.Velocity = bf.Velocity:Lerp(Vector3.new(0, targetRoot.Velocity.Y, 0), SMOOTHNESS)
...
StarterGui:SetCore("SendNotification", {Title = "Hệ thống mạnh nhất lịch sử", Text = "Vui lòng chọn thí chủ!"})
